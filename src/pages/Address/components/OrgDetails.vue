            ...
            <el-form :model="userData" :rules="rules" ref="ruleForm" label-width="160px" class="demo-ruleForm">
                        ...
                        <el-row :gutter="20" class="addDet_InputBox">
                            <el-col :span="16">
                                <el-form-item label="单位在项目中的定位" class="speLabel">
                                    <el-select v-model="userData.typeName" :multiple="false" filterable allow-create
                                        default-first-option placeholder="选择单位在项目中的定位/或自己创建">
                                        <div class="speLabelUlBox">
                                            <el-option v-for="(item,index) in UnitPositList" :key="index" :label="item.name"
                                                :value="item.name" class="speLabelLists">
                                            </el-option>
                                        </div>
                                    </el-select>
                                </el-form-item>
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="17" class="addDet_textRigth" style="text-align:right">
                        ...
                        <el-row :gutter="20" class="addDet_InputBox">
                            <el-col :span="16">
                                <el-form-item v-for="(domain, index) in userData.departmentList" :label="'部门'" :key="index"
                                    v-if="!domain.del">
                                    <el-input v-model="domain.departmentName" :style="{width:'120px'}"></el-input>
    ...
    export default {
        ...
        methods: {
            submitForm(formName) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        //   console.log(this.ruleForm.deleteDepart,this.userData.departmentList)
                        let userInfoList = [
                            ...this.ruleForm.deleteDepart,
                            ...this.userData.departmentList
                        ];
                        ...
                        for (let i = 0; i < userInfoList.length; i++) {
                            ...
                            departmentList[i].departmentName = userInfoList[i].departmentName;
                            if (userInfoList[i].departmentId) {
                                departmentList[i].departmentId = userInfoList[i].departmentId;
                            }

                            if (userInfoList[i].departmentId) {
                                departmentList[i].operation = userInfoList[i].del
                                    ? "delete"
                                    : "edite";
                            } else {
            ...
            removeDomain(item) {
                ...
                if (index !== -1) {
                    this.userData.departmentList[index].del = true;
                    this.ruleForm.deleteDepart.push(this.userData.departmentList[index]);
